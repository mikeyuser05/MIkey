import type { ReactElement } from 'react';
import { BellRing, ShieldAlert, CheckCircle2 } from 'lucide-react';

export function AlertsPage(): ReactElement {
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Alert Decision Engine</h1>
        <p className="text-sm text-slate-500">PR4 Real-time Safety Thresholds & Logs</p>
      </div>

      <div className="p-5 rounded-2xl bg-surface-light dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-3 text-emerald-500 mb-2">
          <CheckCircle2 className="h-5 w-5" />
          <span className="font-semibold text-slate-900 dark:text-white">All Systems Nominal</span>
        </div>
        <p className="text-xs text-slate-400">No active emergency alerts triggered by PR4.3 Alert Engine.</p>
      </div>
    </div>
  );
}
export default AlertsPage;